// AlgoHW7-2.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void dfs(int** graph, int i, bool* visited, int vCount)
{
    visited[i] = true;
    std::cout << "" << i + 1 << " ";
...
void bfs(int** graph, int vCount)
{
    ...
    for (int i = 0; i < vCount; i++)
    {
        if (!visited[i])
        {
            queue[qCount] = i;//номер вершины - в очередь
            qCount++;
