...
//
/*
Заметим, что оператор "+"и унарный "-" а также префиксные инкременты, возвращают ссылку на сам объект, 

поэтому тип их возвращаемого значения - Fraction&.


Между тем операторы постфиксного инкремента и декремента, а также сложения, вычитания, умножения и деления 

возвращают результат по значению,

т.к. результат хранится во временной переменной, которая уничтожается после выхода из функции,

поэтому эти операторы не могут возвращать результат по ссылке.
*/
